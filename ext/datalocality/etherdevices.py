...
from ext.datalocality.device import Device
from ext.datalocality.model import *
    ...
    def create():
        if device.arch is Arch.ARM32:
            ...
            if cpu_mhz or cpu_cores:
                ...
                device.connection = Connection.WIFI
                ...
                device.connection = Connection.ETHERNET
                device.cpu = CpuModel.ARM
                device.network = Bins.LOW
                device.location = Location.EDGE
                return rpi3, device
        elif device.arch is Arch.AARCH64:
            if device.accelerator is Accelerator.NONE:
                ...
                device.cpu = CpuModel.ARM
                device.network = Bins.LOW
                device.location = Location.CLOUD
                device.disk = Disk.SD
...
def create_aarch64_gpu(device):
    adevice = device
    if device.ram is Bins.LOW or device.cpu_mhz is Bins.LOW:
        node = create_nano()
        ...
        tx2_device.connection = Connection.WIFI
